            ...
            xcollector.on("collect", async (interaction) => {
                if (xPlaying.has(message.author.id + message.id)) {
                    if (interaction.user.id === message.author.id) {
                        ...
                        if (a1 === "x" && a2 === "x" && a3 === "x") {
                            initial.edit({ content: `${message.author} wins!\n\nCoins won: <:Coins:946757996166389810>\`${amount * 2} ${tBucks}\`\nCoins lost: <:Coins:946757996166389810> \`${amount}\` ${tBucks}`}).catch((err) => { })
                            economy.addCoins(message.guild.id, message.author.id, amount * 2)
                            economy.addCoins(message.guild.id, opponent.user.id, amount * -1)
                            xcollector.stop()
                    ...
                    if (interaction.user.id === message.author.id) {
                        ...
                        if (a1 === "o" && a2 === "o" && a3 === "o") {
                            ...
                            economy.addCoins(message.guild.id, opponent.user.id, amount * 2)
                            economy.addCoins(message.guild.id, message.author.id, amount * -1)
                            xcollector.stop()
                            xPlaying.delete(opponent.user.id + message.id)
                            xPlaying.delete(message.author.id + message.id)
