...
module.exports = {
    name: "yt-together",
    aliases: ["yt-t", "yttogether", "ytt", "youtube-together"],
    description: "Starts a new Youtube-Together activity in a voice channel.",
        ...
        serverConfig.findOne({ guildId: message.guild.id }, async (err, data) => {
            ...
            fetch(`https://discord.com/api/v9/channels/${channel.id}/invites`, {
                ...
                .then(async (invite) => {
                    if (!invite.code) {
                        try {
                            return await message.reply("Sadly I was unable to start a YouTube Together activity.")
                        } catch (e) {
                            console.log(e)
                        }
                    }
