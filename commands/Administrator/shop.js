...
module.exports = {
    name: "shop",
    description: "Opens the shop in the provided channel.",
    usage: "<channel> <item> <number of items>",
    ...
    serverConfig.findOne({guildId: message.guild.id}, async (err, data) => {
        ...
        if (!args[1]) return message.reply("<:error:946775591460421683> : Please specify the item you want to add!")

        shopSchema.findOne({ guildId: message.guild.id, item: args[1].toLowerCase() }, async (err, data) => {
            if (err) console.log(err)
            if (!data) return message.reply("<:error:946775591460421683> : That item doesn't exist!")
            if (!args[2]) return message.reply("<:error:946775591460421683> : Please specify the number of items you want to sell!")
            if (isNaN(args[2])) return message.reply("<:error:946775591460421683> : Please specify a valid number of items you want to sell!")
